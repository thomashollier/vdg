...
for clip in bpy.data.movieclips:
    ...
    if len(perspTrackers)>3 and len(perspTrackers)%4 == 0:
        print("%s perspective trackers" % (len(perspTrackers)))
        perspTrackers = sorted(perspTrackers)
        perspTrackersDict = {}
        for x in range(int(len(perspTrackers)/4.0)):
            perspTrackersDict[x] = []
            for xx in [x*4+0, x*4+1, x*4+2, x*4+3]:
                perspTrackersDict[x].append(perspTrackers[xx])
        print(perspTrackersDict)
        DO_PERSP=True
